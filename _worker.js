...
async function getAllConfigVless(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const vlessTls = `vless://selopuro\u0040${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=${pathFixed}#${data.isp} (${data.country_code})`;
            const vlessNtls = `vless://selopuro\u0040${hostName}:80?path=${pathFixed}&security=none&encryption=none&host=${hostName}&fp=randomized&type=ws&sni=${hostName}#${data.isp} (${data.country_code})`;
...
PATH    : ${path}
=====================================
TLS  : <button class="button" onclick='copyToClipboard("${vlessTlsFixed}")'><i class="fa fa-clipboard"></i>Copy  </button>

NTLS : <button class="button" onclick='copyToClipboard("${vlessNtlsFixed}")'><i class="fa fa-clipboard"></i>Copy  </button>
=====================================
`;
}
